...
export interface SessionRow {
  ...
  status: string;
  total_drive_duration_sec: number;
  monitoring_duration_sec: number;
  yawn_count: number;
  prolonged_eye_closure_count: number;
  blink_count: number;
...
const defaultPath = () => process.env.DATABASE_PATH ?? resolve(__dirname, "../data/store.json");
...
function load(): DbFile {
  ...
  return JSON.parse(readFileSync(path, "utf-8")) as DbFile;
}
